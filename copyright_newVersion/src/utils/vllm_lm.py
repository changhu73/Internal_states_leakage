...
class VllmLM:
    ...
    def generate(self, prompts=None, prompt_token_ids=None, max_new_tokens=1024, temperature=0.0, **sampleing_kwargs):
        ...
        output_text_list = []
        # We assume model_vllm has access to the model's forward pass
        outputs = self.model_vllm.generate(prompts=prompts, sampling_params=sampling_params, prompt_token_ids=prompt_token_ids)
    ...
    def extract_hidden_states(self, text, tokenizer):
        """
        Extract hidden states from the model for a given input text.
        """
        # Tokenize the input text
        import torch
        inputs = tokenizer(text, return_tensors="pt", padding=True, truncation=True)
        
        # Run the forward pass to get hidden states
        with torch.no_grad():
            # Assuming `model_vllm.model` has access to the transformers-based model
            outputs = self.model_vllm.model(**inputs, output_hidden_states=True)
        
        # Get hidden states
        last_layer_hidden_states = outputs.hidden_states[-1]  # Get the last hidden layer
        last_token_hidden_states = last_layer_hidden_states[:, -1, :]  # Extract hidden state of the last token
        
        hidden_state = last_token_hidden_states.cpu().numpy()  # Ensure it is moved to CPU and converted to numpy array
        return hidden_state
