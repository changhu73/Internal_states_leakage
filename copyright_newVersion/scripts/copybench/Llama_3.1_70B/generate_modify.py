...
def load_classification_model(classifier_model, device):
    # Load the trained classification model (CustomMLP)
    custom_mlp = CustomMLP(input_dim=8960, hidden_dim=256)  # Adjust the input_dim accordingly
    custom_mlp.load_state_dict(torch.load(args.classifier_model))
    custom_mlp.to(device)
    custom_mlp.eval()  # Set to evaluation mode
    return custom_mlp
...
def extract_hidden_states(text, model, tokenizer):
    ...
    hidden_state = last_token_hidden_states.cpu().numpy()  # Ensure the data is on CPU
...
def main(args):
    ...
    custom_mlp = CustomMLP(input_dim=8960, hidden_dim=256)  # Modify input dimensions as needed
    custom_mlp.load_state_dict(torch.load(args.classifier_model))
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    custom_mlp.to(device)
    ...
    if "gpt" in args.model.lower():   # openai api
        ...
        extract_tokenizer = AutoTokenizer.from_pretrained(args.model, model_max_length=512)
            # Handle padding issues
        if extract_tokenizer.pad_token is None:
            # Manually add pad_token
            extract_tokenizer.add_special_tokens({'pad_token': '[PAD]'})
        # Use eos_token as pad_token
        extract_tokenizer.pad_token = extract_tokenizer.eos_token  # Set eos_token as pad_token
        extract_model = load_large_model(args.model)
        ...
        with tqdm(total=len(snippets)) as pbar:
            for i in range(0, len(snippets), args.batch_size):
                ...
                for s, prompt, output_text in zip(snippets[i:i+args.batch_size], prompt_list, output_text_list):
                    extract_states = extract_hidden_states(s["input"], extract_model, tokenizer=extract_tokenizer)
                    label = predict_model(custom_mlp, extract_states, threshold=0.5)
                    if label == 0:
                        output_text = "Sorry, the input has a high risk of text infringement. Generation has been halted."
                    else:
